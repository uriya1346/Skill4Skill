...
function LearningGroupsCatAdmin(props){
let [ar,setAr] = useState([]);
  ...
  const delCategory = async(_idDel) => {
    if(window.confirm("Are you sure you want to delete?")){
      try{
        let url = API_URL+"/categoriesRental/"+_idDel;
  ...
  return(
    <div className='container mb-5'>
      ...
      <h1 className='gradi text-center'><i className="fa fa-lastfm me-4 mb-4" aria-hidden="true"></i>Learning Groups Panel</h1>
      <table className='table table-bordered table-dark table-striped table-responsive flex-md-column-reverse col-lg-6 my-5 text-center'>
        <thead>
          <tr>
            ...
            <th>List<i className="fa fa-list ms-2" aria-hidden="true"></i></th>
          </tr>
        </thead>
        <tbody>
          {ar.map((item,i) => {
            return(
              <tr className='link alert-link' key={item._id}>
                ...
                <td>
                  ...
                  <button onClick={() => {
                    nav("/admin/learningGroups/"+item.url_name)
                  }} className='badge btn btn-outline-success'><i className="fa fa-pencil" aria-hidden="true"></i></button>
                </td>
                <td>
                    <div></div>
                  <button onClick={() => {
                    nav("/admin/groupsList/"+item.url_name)
                  }} className='badge text-center mx-2 btnLog pe-4'><i className="fa fa-arrow-right" aria-hidden="true"></i></button>
                </td>
        ...
        <div className='d-flex justify-content-between mx-md-5'>
          <div>
      <Link to="/admin/addGroupCat" className="btnTable">Add new category<i className="fa fa-plus mx-2 mb-3" aria-hidden="true"></i></Link> 
      </div>
      <div>
      <Link to="/admin/addGroupProduct" className="btnTable">Add new product<i className="fa fa-plus mx-2 mb-3" aria-hidden="true"></i></Link> 
      </div>
      </div>
...
export default LearningGroupsCatAdmin