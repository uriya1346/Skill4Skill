...
function EditProductRental(props) {
  ...
  const doApi = async () => {
    ...
    setCatAr(resp.data);
    let urlProduct = API_URL+"/rental/single/"+params.id;
  ...
  const doFormApi = async (formData) => {
    let url = API_URL + "/rental/"+params.id;
    try {
...
<div style={{ minHeight: "15vh" }}></div>
      ...
      <form onSubmit={handleSubmit(onSubForm)} className='col-md-7 p-3 shadow mx-auto'>
        ...
        <input defaultValue={product.day_price} {...priceRef} type="number"  className='form-control' />
...
export default EditProductRental